#!/usr/bin/env python3
"""
TACCV4 — PAKE-secured pairing over an untrusted relay (Reference Implementation)
...
def check_dependencies():
    ...
    # Import structlog after checking
    global logger
    import structlog
...
# Cryptographic profiles with minimum version
SUPPORTED_PROFILES = ["v40-pake", "v35-pake"]
MIN_PROFILE = "v40-pake"
DEFAULT_PROFILE = "v40-pake"

# Profile ranking for downgrade protection
PROFILE_RANKS = {
    "v40-pake": 40,
    "v35-pake": 35,
}
...
def build_nonce(nonce_base: bytes, seq2: int) -> bytes:
    ...
    # Mix seq2 into nonce_base: first 4 bytes stay, last 8 bytes XOR with seq2
...
def require_phase(cur: Phase, *allowed: Phase):
    if cur not in allowed:
        raise RuntimeError(f"invalid phase={cur}; expected one of {allowed}")
...
def pake_finish_reduced_timing(pake_state: dict, msg_in: bytes, sid: bytes, profile: str):
    SPAKE2_A, SPAKE2_B = require_spake2()
    
    # Attempt to reduce timing signals (not constant-time)
...
def hs_derive_master(hs_state: dict, peer_pub_raw: bytes, peer_nonce: bytes, recon_th: bytes):
    ...
    return {
        ...
        "th": th,
        "nonce_base_send": nonce_base_send,
        "nonce_base_recv": nonce_base_recv,
    }
...
def negotiate_profile(my_profiles: List[str], peer_profiles: List[str]) -> str:
    """Negotiate highest mutually supported profile that meets minimum requirements."""
...
def build_relay_app():
    ...
    app = FastAPI(title="TACCV4 Relay", version=PROTO_VER)
...
class WSClient:
    ...
    async def connect(self):
        """Connect to relay and start background receiver."""
        try:
            import websockets
        except ImportError as e:
    ...
    async def _wait_for_message(self, condition) -> Optional[Dict[str, Any]]:
        """Wait for a message matching condition."""
        ...
        while self._running and (time.time() - start_time < timeout):
            try:
                ...
                logger.error("wait_for_message_error", error=str(e))
                break
        
        return None
    
    def _validate_message_phase(self, kind: str) -> bool:
        """Validate message is allowed in current phase."""
        ...
        if kind == "pake_confirm":
            return phase == Phase.PAKE or phase == Phase.PAKE_DONE
    ...
    async def _process_received_message(self, msg: Dict[str, Any]):
        """Process received message from background queue."""
        msg_type = msg.get("type")
    ...
    async def send_packet(self, kind: str, payload: Dict[str, Any], op: str = "req"):
        """Send protocol packet with op (req/resp) field."""
        self.state.seq_out += 1
        seq = self.state.seq_out
        
        wire_payload = {}
        for k, v in payload.items():
            wire_payload["rid" if k == "id" else k] = v
    ...
    async def _handle_peer(self, from_role: ROLE, msg: Dict[str, Any]):
        """Handle message from peer with phase gating."""
        ...
        if not self._validate_message_phase(kind):
            await self.abort(f"Message {kind} invalid in phase {self.state.phase}", 
                           AbortCode.PROTOCOL_VIOLATION)
            return
        ...
        # Handle connection ID for sequence reset
        if kind == "profile_hello":
        ...
        if REQUIRE_SEQ_MONOTONIC and seq and seq <= self.state.peer_seq_in:
            ...
            if self._out_of_order_count % 5 == 0:
                logger.warning("many_out_of_order", role=self.role, count=self._out_of_order_count)
            return
        ...
        if op == "resp":
            ...
            if req_id in self._pending:
                ...
                logger.warning("unexpected_response", kind=kind, rid=req_id)
                return
    ...
    async def _handle_request(self, kind: str, req_id: str, payload: Dict[str, Any], from_role: ROLE):
        """Respond to protocol requests from peer."""
        inbox_entry = {
            "rid": req_id,
        ...
        if kind == "profile_hello":
            ...
            if self._pake_state:
                ...
                logger.debug("deferred_pake_response", rid=req_id)
                return
    ...
    async def _handle_secure(self, from_role: ROLE, payload: Dict[str, Any]):
        ...
        # Validate ciphertext length (ChaCha20Poly1305 tag is 16 bytes)
        # Minimum: 1 byte plaintext + 16 byte tag = 17 bytes ciphertext
        ct = validate_base64(payload["blob_b64"], "secure_ct", 17)
        
        # FIXED: Bind AAD to actual sender
        aad = secure_aad(
            self.state.sid, self.state.profile, self.state.recon_th,
            self.state.th, from_role, seq2
        )
        ...
        try:
            ...
            if self.state.decrypt_failures >= MAX_DECRYPT_FAILURES:
                await self.abort(f"Too many decrypt failures ({self.state.decrypt_failures})", 
                               AbortCode.CRYPTO_ERROR)
                return
            
            logger.warning("decryption_failed", role=self.role, error=str(e), 
                          failures=self.state.decrypt_failures)
            return
    ...
    async def run(self, demo_message: str = ""):
        ...
        try:
            ...
            negotiated = None
            for profile in sorted(SUPPORTED_PROFILES, key=lambda p: PROFILE_RANKS[p], reverse=True):
                if profile in peer_profiles and profile_allowed(profile):
                    negotiated = profile
                    break
            
            if not negotiated:
                await self.abort("No mutually acceptable profile found", AbortCode.PROFILE_MISMATCH)
            
            if not profile_allowed(negotiated):
                await self.abort(f"Profile {negotiated} below minimum security level", 
                               AbortCode.PROFILE_DOWNGRADE)
            ...
            if inbox_msg:
                ...
                if not req_id:
                    await self.abort("pake_hello missing rid in inbox", AbortCode.INVALID_MESSAGE)
                
                await self.send_packet("pake_hello", {
                    "id": req_id,
                    "msg_b64": b64e(self._pake_state["msg_out"])
                }, op="resp")
            ...
            self.state.k0 = pake_finish_reduced_timing(
                self._pake_state, peer_msg, self.state.sid, negotiated
            )
            self._pake_state = None
            
            self.state.recon_th = self.state.transcript.digest()
            
            my_pake_tag = pake_confirm_tag(
                self.state.k0, self.state.sid, self.role, self.state.recon_th, negotiated
            )
            ...
            exp_peer_pake = pake_confirm_tag(
                self.state.k0, self.state.sid, self.peer, self.state.recon_th, negotiated
            )
            ...
            if inbox_msg:
                req_id = inbox_msg.get("rid")
                if not req_id:
                    await self.abort("hs_hello missing rid in inbox", AbortCode.INVALID_MESSAGE)
                
                await self.send_packet("hs_hello", {
                    "id": req_id,
                    "pub_b64": b64e(self._hs_state["pub_raw"]),
                    "nonce_b64": b64e(self._hs_state["nonce"])
                }, op="resp")
            ...
            self.state.send_key, self.state.recv_key = derive_session_keys(
                self.state.master, self.state.th, self.role, negotiated
            )
            ...
            logger.error("protocol_error", role=self.role, error=str(e))
            raise
        except Exception as e:
            logger.error("unexpected_error", role=self.role, error=str(e))
    ...
    async def send_secure(self, text: str):
        ...
        await self.send_packet("secure_msg", {
            "seq2": seq2,
            "blob_b64": b64e(ct)
        })
...
def security_self_check():
    ...
    if sys.version_info >= (3, 9):
        ...
        checks.append(("Python >= 3.9", False, "Python 3.9+ required"))
    
    try:
        test = [secrets.randbits(16) for _ in range(10)]
        if all(x != 0 for x in test):
            checks.append(("Random source", True))
        else:
            checks.append(("Random source", False, "Weak random detected"))
    except:
    ...
    # Check base64 strictness
    try:
        b64d("aW52YWxpZCBwYWRkaW5n")  # Valid
    ...
    checks.append(("Downgrade protection", profile_allowed("v40-pake") and not profile_allowed("v30-pake")))
    
    all_ok = all(check[1] for check in checks)
    
    for check in checks:
        if check[1]:
            logger.info("security_check", check=check[0], status="OK")
        else:
            logger.error("security_check", check=check[0], status="FAILED", 
                        reason=check[2] if len(check) > 2 else "check failed")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="TACCV4 Secure Pairing Protocol")
    ...
    client_parser.add_argument("--message", default="Hello from TACCV4!")
    ...
    if args.command == "check":
        ...
        print(f"Starting TACCV4 client as {args.role} in session {args.session}")
